...
unsigned int lexikon_get_engineerid(void)
{
	return engineerid;
}
...
static int pm8058_gpios_init(void)
{
	...
	if (rc) {
		...
		pr_info("%s [cm3602][PS]LEXIKON_GPIO_PS_INT_N config ok\n", __func__);
	...
	if (rc) {
		...
		pr_info("%s [cm3602][PS]LEXIKON_GPIO_LS_EN config ok\n", __func__);
	...
	if (rc) {
		...
		pr_info("%s [cm3602][PS]LEXIKON_GPIO_PS_EN config ok\n", __func__);
	...
	if (rc) {
		pr_err("%s PMIC_GPIO_LEXIKON_GREEN_LED config failed\n", __func__);
		return rc;
	}
	...
	if (rc) {
		pr_err("%s LEXIKON_GPIO_COMPASS_INT_N config failed\n", __func__);
		return rc;
	} else
		pr_info("%s [AKM8975] LEXIKON_GPIO_COMPASS_INT_N config ok\n",
				__func__);
...
static unsigned aux_pcm_gpio_off[] = {
	PCOM_GPIO_CFG(138, 0, GPIO_CFG_INPUT, GPIO_PULL_DOWN, GPIO_CFG_2MA),   /* PCM_DOUT */
	PCOM_GPIO_CFG(139, 0, GPIO_CFG_INPUT, GPIO_PULL_DOWN, GPIO_CFG_2MA),   /* PCM_DIN  */
	PCOM_GPIO_CFG(140, 0, GPIO_CFG_INPUT, GPIO_PULL_DOWN, GPIO_CFG_2MA),   /* PCM_SYNC */
	PCOM_GPIO_CFG(141, 0, GPIO_CFG_INPUT, GPIO_PULL_DOWN, GPIO_CFG_2MA),   /* PCM_CLK  */
};
...
static int fm_radio_setup(struct marimba_fm_platform_data *pdata)
{
	...
	irqcfg = PCOM_GPIO_CFG(147, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
					GPIO_CFG_2MA);
...
static void fm_radio_shutdown(struct marimba_fm_platform_data *pdata)
{
	int rc;
	const char *id = "FMPW";
	uint32_t irqcfg = PCOM_GPIO_CFG(147, 0, GPIO_CFG_INPUT, GPIO_PULL_UP,
					GPIO_CFG_2MA);
	rc = gpio_tlmm_config(irqcfg, GPIO_CFG_ENABLE);
	if (rc) {
		printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n",
...
static struct smsc911x_platform_config smsc911x_config = {
	.phy_interflexikon	= PHY_INTERFLEXIKON_MODE_MII,
...
static uint32_t qsd_spi_gpio_on_table[] = {
	PCOM_GPIO_CFG(45, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
	PCOM_GPIO_CFG(47, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
	PCOM_GPIO_CFG(48, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
	PCOM_GPIO_CFG(87, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA)
};

static uint32_t qsd_spi_gpio_off_table[] = {
	PCOM_GPIO_CFG(45, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
	PCOM_GPIO_CFG(47, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
	PCOM_GPIO_CFG(48, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
	PCOM_GPIO_CFG(87, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA)
};
...
void config_lexikon_usb_id_gpios(bool output)
{
	if (output) {
		...
		printk(KERN_INFO "%s %d input none pull\n", __func__, LEXIKON_GPIO_USB_ID1_PIN);
	}
}
...
static struct htc_sleep_clk_platform_data htc_slp_clk_data = {
	.sleep_clk_pin = LEXIKON_GPIO_WIFI_BT_SLEEP_CLK_EN,
...
static void __init lexikon_init_irq(void)
{
	msm_init_irq();
}
#if 0
...
static int mbp_config_interflexikon_mode(int state)
{
	if (state) {
...
static int mbp_power_up(void)
{
	...
	udelay(300);
	rc = mbp_config_interflexikon_mode(MBP_ON);
	if (rc)
		goto exit;
	pr_debug("%s: mbp_config_interflexikon_mode() done\n", __func__);
...
/* HTC_WIFI_START */
/*
static unsigned int lexikon_sdc3_slot_type = MMC_TYPE_SDIO_WIFI;
static struct mmc_platform_data msm7x30_sdc3_data = {
	...
	.msmsdcc_fmid	= 24576000,
	.msmsdcc_fmax	= 49152000,
	.slot_type		= &lexikon_sdc3_slot_type,
	.nonremovable	= 0,
};
* */
/* HTC_WIFI_END */
...
static void lexikon_te_gpio_config(void)
{
	...
	PCOM_GPIO_CFG(30, 1, GPIO_CFG_INPUT, GPIO_PULL_DOWN, GPIO_CFG_2MA),
	};
